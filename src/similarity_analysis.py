...
def plot_embeddings_old(embeddings, color_by, title, color_map):
    ...
    plt.tick_params(axis='both', direction='in', which='both', width=1, fontsize=18)
    
    filename = title.lower().replace(' ', '_')
    plt.legend(legend_elements, category_labels, fontsize=20, loc="upper right")
...
def plot_embeddings(embeddings, color_by, title, color_map):
    ...
    unique_categories = np.unique(category_codes)
    legend_elements = [
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=scatter.cmap(scatter.norm(code)), markersize=10)
        for code in unique_categories
    ]

    # Set ticks inside the plot and make them thicker
    plt.tick_params(axis='both', direction='in', which='both', width=1, labelsize=18)

    # Adjust ncol to prevent legend from being too wide
    max_columns = 3  # Set a reasonable maximum number of columns
    ncols = max_columns
    
    # Add legend below the plot in multiple lines if necessary
    plt.legend(
        legend_elements, category_labels, fontsize=20, loc="upper center", 
        bbox_to_anchor=(0.5, -0.10), ncol=ncols, title="Research area", title_fontsize=22, frameon=True
    )

    # Save the figure
    filename = title.lower().replace(' ', '_')
    plt.savefig(f"plots/rq2/{filename}.svg", format="svg", bbox_inches="tight", pad_inches=0.1)
    plt.savefig(f"plots/rq2/{filename}.png", format="png", dpi=300, bbox_inches="tight", pad_inches=0.1)
    plt.savefig(f"plots/rq2/{filename}.pdf", format="pdf", bbox_inches="tight", pad_inches=0.1)
...
if True:
    ...
    plot_embeddings(reduced_embeddings_abstracts, df['main_collection_area'], 'Reference Research Publication Abstracts \n Sentence-BERT Embeddings — t-SNE', 'plasma')
